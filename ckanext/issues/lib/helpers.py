...
def replace_url_param(
    new_params, alternative_url=None, controller=None, action=None, extras=None
):
    """
    ...
    """
    params_cleaned = [
        (k, v) for k, v in toolkit.request.params.items() if k not in new_params.keys()
    ]
...
class Pagination(object):
    ...
    def iter_pages(self):
        for i in range(self.page - self.show_left, self.page + self.show_right + 1):
...
def get_issues_per_page():
    try:
        issues_per_page = [int(i) for i in config["ckan.issues.issues_per_page"]]
...
def issues_enabled(dataset):
    ...
    if datasets_with_issues_enabled or organizations_with_issues_enabled:
        if (
            datasets_with_issues_enabled
            and dataset["name"] in datasets_with_issues_enabled
        ):
            return True
        elif (
            organizations_with_issues_enabled
            and (dataset.get("organization") or {}).get("name")
            in organizations_with_issues_enabled
        ):
            return True
        return False
    else:
...
def issues_enabled_for_organization(organization):
    ...
    if organizations_with_issues_enabled:
        return (
            organization
            and organization.get("name") in organizations_with_issues_enabled
        )
    return True
...
def issues_users_who_reported_issue(abuse_reports):
    ...
    for user_id in abuse_reports:
        try:
            users.append(toolkit.get_action("user_show")(data_dict={"id": user_id}))
...
def get_issue_subject(issue):
    ...
    return toolkit._("[{site_title} Issue] {dataset}").format(
...
def issue_comment_is_owner(user, comment_id, issue_number, dataset_id):
    context = {
        "model": model,
        "user": user,
    }
