...
def issue_show(context, data_dict):
    ...
    issue = issuemodel.Issue.get_by_name_or_id_and_number(
        dataset_name_or_id=dataset_id, issue_number=issue_number, session=session
    )
    if not issue:
    ...
    if user:
        try:
            can_edit = p.toolkit.check_access(
                "package_update", context, data_dict={"id": issue.dataset_id}
            )
    ...
    if issue.visibility != "visible" and not can_edit:
        raise p.toolkit.ObjectNotFound(p.toolkit._("Issue marked as spam/abuse"))
    ...
    for comment in issue.comments:
        ...
        if include_reports:
            comment_dict["abuse_reports"] = _add_reports(
                comment, can_edit, context["user"]
            )
...
def _get_next_issue_number(session, dataset_id):
    q = (
        session.query(issuemodel.Issue)
...
def _get_comment_email_body(comment, issue_subject, user_obj, recipient):
    extra_vars = _get_issue_vars(comment.issue, issue_subject, user_obj, recipient)
    extra_vars["comment"] = comment
    return render("issues/email/new_comment.html", extra_vars=extra_vars)
...
def issue_update(context, data_dict):
    ...
    status_change = data_dict.get("status") and (
        data_dict.get("status") != issue.status
    )
    ...
    if status_change:
        if data_dict["status"] == issuemodel.ISSUE_STATUS.closed:
            ...
            user = context["user"]
            user_dict = p.toolkit.get_action("user_show")(data_dict={"id": user})
...
def issue_delete(context, data_dict):
    ...
    issue = issuemodel.Issue.get_by_number(
        dataset_id=dataset_id, issue_number=issue_number, session=session
    )
...
def issue_search(context, data_dict):
    ...
    if organization_id:
        try:
            p.toolkit.check_access(
                "organization_update", context, data_dict={"id": organization_id}
            )
        ...
        try:
            p.toolkit.check_access(
                "package_update", context, data_dict={"id": dataset_id}
            )
            visibility = data_dict.get("visibility", None)
            can_update = True
        except p.toolkit.NotAuthorized:
    ...
    query = issuemodel.Issue.get_issues(session=context["session"], **data_dict)
...
def issue_comment_create(context, data_dict):
    ...
    if notifications:
        ...
        for recipient in recipients:
            body = _get_comment_email_body(issue_comment, subject, user_obj, recipient)
...
def issue_comment_delete(context, data_dict):
    ...
    issuemodel.IssueComment.delete(data_dict["comment_id"])
...
def organization_users_autocomplete(context, data_dict):
    ...
    query = (
        session.query(model.User.id, model.User.name, model.User.fullname)
...
def issue_report(context, data_dict):
    ...
    return _comment_or_issue_report(
        issue, context["user"], data_dict["dataset_id"], session
    )
...
def _comment_or_issue_report(issue_or_comment, user_ref, dataset_id, session):
    ...
    try:
        ...
        p.toolkit.check_access("package_update", context, data_dict={"id": dataset_id})

        issue_or_comment.change_visibility(session, "hidden")
        issue_or_comment.abuse_status = issuemodel.AbuseStatus.abuse.value
        return {
            "visibility": issue_or_comment.visibility,
        ...
        if max_strikes and len(issue_or_comment.abuse_reports) >= p.toolkit.asint(
            max_strikes
        ):
            issue_or_comment.change_visibility(session, "hidden")
    finally:
...
def issue_comment_report(context, data_dict):
    ...
    return _comment_or_issue_report(
        comment, context["user"], comment.issue.dataset_id, session
    )
...
def issue_report_show(context, data_dict):
    ...
    try:
        ...
        p.toolkit.check_access(
            "package_update", package_context, data_dict={"id": dataset_id}
        )
        reports = issuemodel.Issue.Report.get_reports(session, parent_id=issue.id)
    except p.toolkit.NotAuthorized:
        reports = issuemodel.Issue.Report.get_reports_for_user(
            session, user_id=user_id, parent_id=issue.id
        )
...
def issue_report_clear(context, data_dict):
    ...
    dataset_id = data_dict["dataset_id"]
    issue = issuemodel.Issue.get_by_name_or_id_and_number(
        session=session, dataset_name_or_id=dataset_id, issue_number=issue_number
    )
    ...
    try:
        ...
        if max_strikes and len(issue.abuse_reports) <= p.toolkit.asint(max_strikes):
            issue.change_visibility(session, "visible")
    finally:
        session.commit()
    return True
...
def issue_comment_report_clear(context, data_dict):
    ...
    try:
        ...
        max_strikes = config.get("ckanext.issues.max_strikes")
        if max_strikes and len(comment.abuse_reports) <= p.toolkit.asint(max_strikes):
            comment.change_visibility(session, "visible")
    finally:
        session.commit()
    return True
