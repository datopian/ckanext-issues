        ...
        return {
            ...
            "msg": p.toolkit._(
                "User {0} not authorized for action on issue {1}".format(
                    str(context["user"]), auth_data_dict["id"]
                )
            ),
        }
...
def issue_search(context, data_dict):
    try:
        ...
        return {
            "success": False,
            "msg": p.toolkit._(
                "User {0} not authorized for action".format(str(context["user"]))
            ),
        }
...
def issue_update(context, data_dict):
    ...
    if data_dict.get("assignee_id") or data_dict.get("can_edit"):
        creator = issue.user_id == user_obj.id
        out = issue_auth(context, data_dict, "package_update")
        if out["success"] or creator:
            return {"success": True}
        else:
    ...
    return {
        ...
        "msg": p.toolkit._(
            "User {user} not authorized for action on issue {issue}".format(
                user=str(user), issue=data_dict["issue_number"]
            )
        ),
    }
